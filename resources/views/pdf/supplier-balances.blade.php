...
<head>
    ...
    <title>Saldos de Proveedores</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="header-right">
                ...
                <div class="report-title">Saldos de Proveedores</div>
        ...
        <div class="summary-container">
            <div class="summary-card">
                <div class="summary-box suppliers">
                    ...
                    <div class="summary-value suppliers">{{ $totals['suppliers_count'] ?? 0 }}</div>
                    <div class="summary-sub">{{ $totals['suppliers_with_debt'] ?? 0 }} con deuda</div>
                </div>
            </div>
            <div class="summary-card">
                <div class="summary-box purchases">
                    <div class="summary-label">Total Compras</div>
                    <div class="summary-value purchases">${{ number_format($totals['total_purchases'] ?? 0, 0, ',', '.') }}</div>
                </div>
            </div>
            <div class="summary-card">
            ...
            <div class="summary-card">
                <div class="summary-box pending">
                    ...
                    <div class="summary-value pending">${{ number_format($totals['total_pending'] ?? 0, 0, ',', '.') }}</div>
                </div>
            </div>
        </div>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 10%;">NIT/RUT</th>
                    <th class="right" style="width: 15%;">Total Compras</th>
                    <th class="right" style="width: 15%;">Total Pagado</th>
                    <th class="right" style="width: 15%;">Pendiente</th>
                    <th style="width: 10%;">Últ. Compra</th>
                    <th style="width: 10%;">Últ. Pago</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="right amount-paid">
                        ${{ number_format($supplier['total_paid'] ?? 0, 0, ',', '.') }}
                    </td>
                    <td class="right amount-pending">
                        ${{ number_format($supplier['total_pending'] ?? 0, 0, ',', '.') }}
                    </td>
                    <td>
                        @if(!empty($supplier['last_purchase_date']))
                            {{ \Carbon\Carbon::parse($supplier['last_purchase_date'])->format('d/m/Y') }}
                        @else
                            -
                        @endif
                    </td>
                    <td>
                        @if(!empty($supplier['last_payment_date']))
                            {{ \Carbon\Carbon::parse($supplier['last_payment_date'])->format('d/m/Y') }}
                        @else
                            -
                        @endif
                    </td>
                    <td class="center">
                        @php
                            $status = $supplier['payment_status'] ?? 'pending';
        ...
        @else
        <p style="text-align: center; color: #6B7280; padding: 20px;">No hay proveedores con compras registradas</p>
        @endif
