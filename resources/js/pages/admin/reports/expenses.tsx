...
export default function ExpensesReportPage() {
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(79, 70, 229);
      pdf.text('Informe de Gastos', rightX, currentY + 9, { align: 'right' });
      ...
      // ── Summary Cards ──
      const avgExpense = report.totals.expense_count > 0 ? report.totals.total_expense / report.totals.expense_count : 0;
      const cardData = [
        { label: 'TOTAL GASTOS', value: formatCurrency(report.totals.total_expense), bg: [254, 242, 242], border: [252, 165, 165], color: [220, 38, 38] },
        { label: 'CANTIDAD DE GASTOS', value: String(report.totals.expense_count), bg: [238, 242, 255], border: [199, 210, 254], color: [37, 99, 235] },
        { label: 'PROMEDIO POR GASTO', value: formatCurrency(avgExpense), bg: [255, 251, 235], border: [253, 230, 138], color: [217, 119, 6] },
      ];
      ...
      // ── Charts ──
      const chartsContainer = document.getElementById('expenses-charts-container');
      if (chartsContainer) {
        try {
      ...
      const tableHead = [['# Pago', 'Fecha', 'Monto', 'Método de Pago', 'Caja', 'Concepto', 'Creado por']];
      ...
      tableBody.push([
        'TOTALES', '', `-${formatCurrency(report.totals.total_expense)}`, '', '', '', '',
      ]);
      ...
      addFooters();
      pdf.save(`Gastos_${filters.date_from}_${filters.date_to}.pdf`);
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      ...
      // ── Report header ──
      rows.push([`INFORME DE GASTOS — ${companyName}`, '', '', '', '', '', '']);
      ...
      rows.push([
        'RESUMEN',
        `Total Gastos: ${formatCurrency(report.totals.total_expense)}`,
        '',
        `Cantidad: ${report.totals.expense_count}`,
        '',
        `Promedio: ${formatCurrency(avgExpense)}`,
        '',
      ]);
      sectionHeaderRows.push(row);
      row++;
      ...
      ws['!cols'] = [
        { wch: 14 }, { wch: 16 }, { wch: 18 }, { wch: 18 }, { wch: 16 }, { wch: 28 }, { wch: 18 },
      ];
      ...
      for (let R = 0; R < rows.length; R++) {
        for (let C = 0; C < totalCols; C++) {
          ...
          if (!sectionHeaderRows.includes(R) && !columnHeaderRows.includes(R) && R !== 1 && R !== expensesTotalsRow && C === 2) {
            ws[addr].s = {
              ...ws[addr].s,
      ...
      XLSX.writeFile(wb, `Gastos_${filters.date_from}_${filters.date_to}.xlsx`);
  ...
  const loadReport = async () => {
    ...
    try {
      setLoading(true);
      setGeneralError("");
      const data = await reportsApi.expenses({
  ...
  const methodPieData = useMemo(() => {
    ...
    report.items.forEach(i => {
      const key = i.payment_method_name || "Sin método";
  ...
  return (
    <AppLayout title="Informe de Gastos">
      <Head title="Informe de Gastos" />
      ...
      <div className="min-h-screen bg-background -mx-2 sm:-mx-4 lg:-mx-6 -mt-6">
          ...
          <div className="max-w-[1400px] mx-auto px-4 py-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-semibold text-foreground">Informe de Gastos</h1>
                  <p className="text-sm text-muted-foreground">Listado de todos los egresos registrados en el periodo</p>
                </div>
              </div>
              {report && (
        ...
        <div className="max-w-[1400px] mx-auto px-4 py-6 space-y-6">
              ...
              <Card className="bg-card/50 backdrop-blur-sm border-2 border-border hover:shadow-lg transition-shadow">
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-sm font-medium text-muted-foreground">Cantidad de Gastos</h3>
                  </div>
              ...
              <Card className="bg-card/50 backdrop-blur-sm border-2 border-border hover:shadow-lg transition-shadow">
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-sm font-medium text-muted-foreground">Promedio por Gasto</h3>
                  </div>
                  <p className="text-2xl font-bold text-amber-600">
                    {formatCurrency(report.totals.expense_count > 0 ? report.totals.total_expense / report.totals.expense_count : 0)}
                  </p>
                </div>
              </Card>
            </div>
          ...
          {report && (
            <>
              {/* Charts */}
              <div id="expenses-charts-container" className="grid gap-4 md:grid-cols-3">
                ...
                <Card ref={(el: HTMLDivElement | null) => { chartRefs.current['register-bar'] = el; }} className="shadow-sm border border-border">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Gastos por Caja</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={registerData} margin={{ bottom: 20 }}>
                        ...
                        <Tooltip formatter={(value: number) => formatCurrency(value)} />
                        <Bar dataKey="value" fill="#ef4444" name="Monto" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                <Card ref={(el: HTMLDivElement | null) => { chartRefs.current['daily-area'] = el; }} className="shadow-sm border border-border">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Evolución Diaria de Gastos</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={250}>
                      <AreaChart data={dailyData}>
                        ...
                        <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} fontSize={11} />
                        <Tooltip formatter={(value: number) => formatCurrency(value)} />
                        <Area type="monotone" dataKey="value" stroke="#ef4444" fill="#ef4444" fillOpacity={0.15} name="Gastos" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                ...
                <CardContent>
                  {report.items.length === 0 ? (
                    <div className="text-center py-12 text-muted-foreground">
                      ...
                      <p>No se encontraron gastos en el periodo seleccionado</p>
                    </div>
                  ) : (
            ...
            <Card className="shadow-xl border border-border">
              <CardContent className="py-16">
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-sm">Los resultados aparecerán aquí con gráficas y detalle de gastos</p>
                </div>
              </CardContent>
            </Card>
          )}
