...
export default function SalePriceHistoryReport() {
  ...
  const loadReport = async () => {
    ...
    try {
      setLoading(true);
      setGeneralError("");
      const params: Parameters<typeof reportsApi.salePriceHistory>[0] = {
      ...
      const data = await reportsApi.salePriceHistory(params);
      setReport(data);
    } catch (error: any) {
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(79, 70, 229);
      pdf.text('Historial de Precios de Venta', rightX, currentY + 9, { align: 'right' });
      ...
      // ── Summary Cards ──
      const cardData = [
        { label: 'TOTAL CAMBIOS', value: String(report.totals.total_changes), bg: [239, 246, 255], border: [147, 197, 253], color: [37, 99, 235] },
        { label: 'PRODUCTOS AFECTADOS', value: String(report.totals.products_affected), bg: [245, 243, 255], border: [196, 181, 253], color: [124, 58, 237] },
        { label: 'SUBIDAS', value: String(report.totals.increases), bg: [240, 253, 244], border: [134, 239, 172], color: [22, 163, 74] },
        { label: 'BAJADAS', value: String(report.totals.decreases), bg: [254, 242, 242], border: [252, 165, 165], color: [220, 38, 38] },
      ];
      ...
      // ── Charts ──
      const chartsContainer = document.getElementById('price-history-charts-container');
      if (chartsContainer) {
        try {
      ...
      pdf.text('Detalle de Cambios de Precio', margin + 4, currentY + 4.8);
      currentY += 9;

      const tableHead = [['Fecha', 'Producto', 'SKU', 'Categoría', 'Precio Anterior', 'Precio Nuevo', 'Cambio $', 'Cambio %', 'Razón', 'Usuario']];
      ...
      addFooters();
      pdf.save(`Historial_Precios_Venta_${filters.date_from}_${filters.date_to}.pdf`);
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      ...
      // ── Report header ──
      rows.push([`HISTORIAL DE PRECIOS DE VENTA — ${companyName}`, '', '', '', '', '', '', '', '', '']);
      ...
      // ── Detail section ──
      rows.push(['DETALLE DE CAMBIOS DE PRECIO', '', '', '', '', '', '', '', '', '']);
      sectionHeaderRows.push(row);
      row++;
      rows.push(['Fecha', 'Producto', 'SKU', 'Categoría', 'Precio Anterior', 'Precio Nuevo', 'Cambio $', 'Cambio %', 'Razón', 'Usuario']);
      ...
      rows.push([
        'TOTALES', '', '', '', '', '', `${report.totals.total_changes} cambios`, '', '', '',
      ]);
      row++;
      ...
      ws['!cols'] = [
        { wch: 16 }, { wch: 24 }, { wch: 14 }, { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 12 }, { wch: 20 }, { wch: 16 },
      ];
      ...
      for (let R = 0; R < rows.length; R++) {
        for (let C = 0; C < totalCols; C++) {
          ...
          if (!sectionHeaderRows.includes(R) && !columnHeaderRows.includes(R) && R !== 1 && R !== detailTotalsRow && (C === 4 || C === 5 || C === 6)) {
            ws[addr].s = {
              ...ws[addr].s,
          ...
          // Center-align change % column
          if (!sectionHeaderRows.includes(R) && !columnHeaderRows.includes(R) && R !== 1 && R !== detailTotalsRow && C === 7) {
            ws[addr].s = {
              ...ws[addr].s,
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Historial Precios Venta');
      XLSX.writeFile(wb, `Historial_Precios_Venta_${filters.date_from}_${filters.date_to}.xlsx`);
  ...
  return (
    <AppLayout title="Historial Precios Venta">
      <Head title="Historial de Precios de Venta" />
          ...
          <div className="max-w-[1400px] mx-auto px-4 py-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-semibold text-foreground">Historial de Precios de Venta</h1>
                  <p className="text-sm text-muted-foreground">Seguimiento de cambios en los precios de venta de productos</p>
                </div>
              </div>
              {report && (
        ...
        <div className="max-w-[1400px] mx-auto px-4 py-6 space-y-6">
          ...
          {report && (
            <>
              {/* Charts */}
              {chartData.length > 0 && (
                <div id="price-history-charts-container">
                  <Card ref={(el: HTMLDivElement | null) => { chartRefs.current['changes-bar'] = el; }} className="shadow-sm border border-border">
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm">Productos con Más Cambios de Precio</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <CardContent>
                  {report.items.length === 0 ? (
                    <div className="text-center py-12 text-muted-foreground">
                      ...
                      <p>No se encontraron cambios de precio en este periodo</p>
                    </div>
                  ) : (
                        ...
                        <TableBody>
                          {report.items.map((item) => (
                            <TableRow key={item.id}>
                              ...
                              <TableCell>{item.category_name || "Sin categoría"}</TableCell>
            ...
            <Card className="shadow-xl border border-border">
              <CardContent className="py-16">
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-sm">Los resultados aparecerán aquí con gráficas y detalle de cambios de precio</p>
                </div>
              </CardContent>
            </Card>
          )}
