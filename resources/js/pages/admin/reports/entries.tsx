...
export default function EntriesReportPage() {
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(79, 70, 229);
      pdf.text('Informe de Entradas', rightX, currentY + 9, { align: 'right' });
      ...
      // ── Summary Cards ──
      const avgEntry = report.totals.income_count > 0 ? report.totals.total_income / report.totals.income_count : 0;
      const cardData = [
        { label: 'TOTAL ENTRADAS', value: formatCurrency(report.totals.total_income), bg: [236, 253, 245], border: [167, 243, 208], color: [5, 150, 105] },
        { label: 'CANTIDAD DE ENTRADAS', value: String(report.totals.income_count), bg: [238, 242, 255], border: [199, 210, 254], color: [37, 99, 235] },
        { label: 'PROMEDIO POR ENTRADA', value: formatCurrency(avgEntry), bg: [250, 245, 255], border: [233, 213, 255], color: [147, 51, 234] },
      ];
      ...
      // ── Charts ──
      const chartsContainer = document.getElementById('entries-charts-container');
      if (chartsContainer) {
        try {
          ...
          for (const key of ['method-pie', 'register-bar', 'daily-area']) {
      ...
      pdf.text('Detalle de Entradas', margin + 4, currentY + 4.8);
      currentY += 9;

      const tableHead = [['# Pago', 'Fecha', 'Monto', 'Metodo de Pago', 'Caja', 'Referencia', 'Concepto', 'Creado por']];
      ...
      tableBody.push([
        'TOTALES', '', formatCurrency(report.totals.total_income), '', '', '', '', '',
      ]);
      ...
      addFooters();
      pdf.save(`Entradas_${filters.date_from}_${filters.date_to}.pdf`);
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      ...
      // ── Report header ──
      rows.push([`INFORME DE ENTRADAS — ${companyName}`, '', '', '', '', '', '', '']);
      ...
      rows.push([
        'RESUMEN',
        `Total: ${formatCurrency(report.totals.total_income)}`,
        '',
        `Cantidad: ${report.totals.income_count}`,
        '',
        `Promedio: ${formatCurrency(avgEntry)}`,
        '', '',
      ]);
      sectionHeaderRows.push(row);
      row++;
      ...
      // ── Entries detail section ──
      rows.push(['DETALLE DE ENTRADAS', '', '', '', '', '', '', '']);
      sectionHeaderRows.push(row);
      row++;
      rows.push(['# Pago', 'Fecha', 'Monto', 'Metodo de Pago', 'Caja', 'Referencia', 'Concepto', 'Creado por']);
      ...
      ws['!cols'] = [
        { wch: 14 }, { wch: 16 }, { wch: 18 }, { wch: 18 }, { wch: 16 }, { wch: 18 }, { wch: 24 }, { wch: 16 },
      ];
      ...
      for (let R = 0; R < rows.length; R++) {
        for (let C = 0; C < totalCols; C++) {
          ...
          if (!sectionHeaderRows.includes(R) && !columnHeaderRows.includes(R) && R !== 1 && R !== entriesTotalsRow && C >= 1) {
            ws[addr].s = {
              ...ws[addr].s,
      ...
      XLSX.writeFile(wb, `Entradas_${filters.date_from}_${filters.date_to}.xlsx`);
  ...
  const loadReport = async () => {
    ...
    try {
      setLoading(true);
      setGeneralError("");
      const data = await reportsApi.entries({
  ...
  const registerData = useMemo(() => {
    ...
    const map = new Map<string, number>();
    report.items.forEach(i => {
      const key = i.cash_register_name || "Sin caja";
      map.set(key, (map.get(key) || 0) + i.amount);
    });
    return Array.from(map, ([name, value]) => ({ name, value }))
      .sort((a, b) => b.value - a.value);
  ...
  const dailyData = useMemo(() => {
    ...
    report.items.forEach(i => {
      const curr = map.get(i.payment_date) || { date: i.payment_date, value: 0 };
      curr.value += i.amount;
    ...
    return Array.from(map.values())
      ...
      .map(d => ({
        name: new Date(d.date + "T12:00:00").toLocaleDateString("es-CO", { timeZone: "America/Bogota", day: "numeric", month: "short" }),
        value: d.value,
      }));
  }, [report]);
  ...
  return (
    <AppLayout title="Informe de Entradas">
      <Head title="Informe de Entradas" />
      ...
      <div className="min-h-screen bg-background -mx-2 sm:-mx-4 lg:-mx-6 -mt-6">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="bg-[#2463eb]/10 p-2.5 rounded-lg">
                  <ArrowDownCircle className="h-5 w-5 text-[#2463eb]" />
                </div>
                <div>
                  <h1 className="text-xl font-semibold text-foreground">Informe de Entradas</h1>
                  <p className="text-sm text-muted-foreground">Listado de todos los ingresos registrados en el periodo</p>
                </div>
              </div>
              {report && (
        ...
        <div className="max-w-[1400px] mx-auto px-4 py-6 space-y-6">
              ...
              <Card className="bg-card/50 backdrop-blur-sm border-2 border-border hover:shadow-lg transition-shadow">
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-sm font-medium text-muted-foreground">Cantidad de Entradas</h3>
                  </div>
              ...
              <Card className="bg-card/50 backdrop-blur-sm border-2 border-border hover:shadow-lg transition-shadow">
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-sm font-medium text-muted-foreground">Promedio por Entrada</h3>
                  </div>
                  <p className="text-2xl font-bold text-purple-600">
                    {formatCurrency(report.totals.income_count > 0 ? report.totals.total_income / report.totals.income_count : 0)}
                  </p>
                </div>
              </Card>
            </div>
          ...
          {report && (
            <>
              {/* Charts */}
              <div id="entries-charts-container" className="grid gap-4 md:grid-cols-3">
                <Card ref={(el: HTMLDivElement | null) => { chartRefs.current['method-pie'] = el; }} className="shadow-sm border border-border">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Entradas por Metodo de Pago</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={250}>
                      <PieChart>
                        <Pie data={methodPieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={2} strokeWidth={0}>
                ...
                <Card ref={(el: HTMLDivElement | null) => { chartRefs.current['register-bar'] = el; }} className="shadow-sm border border-border">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Entradas por Caja</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={registerData} margin={{ bottom: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="name" fontSize={10} angle={-25} textAnchor="end" height={50} />
                        <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} fontSize={11} />
                        <Tooltip formatter={(value: number) => formatCurrency(value)} />
                        <Bar dataKey="value" fill="#16a34a" name="Monto" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                <Card ref={(el: HTMLDivElement | null) => { chartRefs.current['daily-area'] = el; }} className="shadow-sm border border-border">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Evolucion Diaria de Entradas</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={250}>
                      <AreaChart data={dailyData}>
                        ...
                        <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} fontSize={11} />
                        <Tooltip formatter={(value: number) => formatCurrency(value)} />
                        <Area type="monotone" dataKey="value" stroke="#16a34a" fill="#16a34a" fillOpacity={0.15} name="Entradas" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                ...
                <CardContent>
                  {report.items.length === 0 ? (
                    <div className="text-center py-12 text-muted-foreground">
                      ...
                      <p>No se encontraron entradas en el periodo seleccionado</p>
                    </div>
                  ) : (
            ...
            <Card className="shadow-xl border border-border">
              <CardContent className="py-16">
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-sm">Los resultados apareceran aqui con graficas y detalle de entradas</p>
                </div>
              </CardContent>
            </Card>
          )}
