...
class StoreServiceRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $companyId = $user->isSuperAdmin()
            ? ($this->input('company_id') ?? $user->company_id)
            : $user->company_id;
        ...
        return [
            ...
            'slug' => [
                ...
                'max:255',
                'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/',
                Rule::unique('services', 'slug')->where(function ($query) use ($companyId) {
            ...
            // company_id se asigna automáticamente en prepareForValidation
            // Solo super admin puede enviarlo explícitamente
            'company_id' => $this->user()->isSuperAdmin()
    ...
    {
        // Forzar is_active = true en creación
        $this->merge([
            'is_active' => true,
        ]);

        // Valor por defecto para unit
        if (!$this->has('unit') || !$this->input('unit')) {
            $this->merge([
                'unit' => 'servicio',
            ]);
        }
